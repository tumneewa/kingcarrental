...
import { SHOP_DEPOSIT_AMOUNT, SHOP_LOGO_URL, SHOP_ADDRESS } from "../../lib/shopConfig";
import { t, useLang, localeFor } from "../../lib/i18n";
import LangSwitcher from "../../components/LangSwitcher";
import ContactFloating from "../../components/ContactFloating";
import { subscribeToPush } from "../../lib/pushClient";
import PhotoThumb from "../../components/PhotoThumb";
import { Phone, User, Clock, MapPin, Loader2, CheckCircle2, Car as CarIcon, Users, DoorClosed, Settings2, Briefcase, Bell } from "lucide-react";
...
function BookingContent() {
  ...
  const [tappedDayNote, setTappedDayNote] = useState("");
  ...
  const depositAmount = SHOP_DEPOSIT_AMOUNT; // ค่ามัดจำ — จ่ายสดที่ร้านตอนรับรถ พร้อมค่าเช่า
  ...
  const handleDayClick = (iso, disabled, note) => {
    setTappedDayNote(note || "");
    if (disabled) return;
    ...
    return (
      <div className="paper-bg flex min-h-screen items-center justify-center p-4">
        <div className="w-full max-w-sm rounded-xl bg-white p-6 text-center shadow-sm">
          ...
          <div className="mt-5 rounded-lg border border-black/5 p-4 text-left" style={{ background: PAPER }}>
            ...
            <div className="mt-1 flex justify-between border-t border-dashed border-black/10 pt-1 font-semibold">
              ...
              <span style={{ color: RED }}>{money(confirmedTotal + confirmedInsurance + confirmedDeposit)}</span>
            </div>
            {confirmedInsurance > 0 && (
              <p className="mt-2 text-[10px] text-stone-400">{t(lang, "damageInsuranceNote", { amount: money(confirmedInsurance) })}</p>
            )}
            <p className="mt-2 text-[10px] text-stone-400">{t(lang, "payAtShopNote")}</p>
          </div>
          ...
          <button
            onClick={() => { setDone(false); setSelectedCarId(null); setName(""); setPhone(""); setRangeStart(null); setRangeEnd(null); setBookingId(null); setSubmitError(""); setHasLicense(false); setPickupLocation(""); setReturnLocation(""); setPushStatus(""); }}
          ...
          <div className="overflow-hidden rounded-xl bg-white shadow-sm">
            ...
            <div className="mt-1 grid grid-cols-7 gap-1">
              {grid.map((d, i) => {
                ...
                const dayNote = tooltipParts.length > 0 ? tooltipParts.join(" · ") : (booking ? t(lang, "tooltipFullyBooked") : "");
                return (
                  <button key={i} type="button" onClick={() => handleDayClick(iso, disabled, dayNote)} disabled={disabled && !dayNote}
                    ...
                    style={{ background: bg, color, cursor: disabled && !dayNote ? "not-allowed" : "pointer" }}>
                    {d.getDate()}
            ...
            <p className="mt-1 text-[10px] text-stone-300 sm:hidden">{t(lang, "tapForInfo")}</p>
            {tappedDayNote && (
              <div className="mt-2 flex items-start gap-1.5 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-[11px] font-medium text-amber-700">
                <span>ℹ️</span>
                <span>{tappedDayNote}</span>
              </div>
            )}
            ...
            {rangeStart && (
              <div className="mt-4 rounded-lg border border-black/5 p-3" style={{ background: PAPER }}>
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => { setRangeStart(null); setRangeEnd(null); setTappedDayNote(""); }} className="text-[11px] text-stone-400 hover:text-stone-600">{t(lang, "clear")}</button>
                </div>
                ...
                {rangeEnd && (
                  <form onSubmit={submitRequest} className="mt-3 space-y-2.5">
                    ...
                    <div className="rounded-lg border border-black/10 bg-white p-3 text-left text-[11px]">
                      <div className="flex justify-between text-stone-500">
                        <span>{t(lang, "estimatedTotal")}</span>
                      ...
                      <div className="mt-1 flex justify-between border-t border-dashed border-black/10 pt-1 font-semibold">
                        <span style={{ color: INK }}>{t(lang, "dueAtPickup")}</span>
                        <span style={{ color: RED }}>{money(totalSel + damageInsuranceAmount + depositAmount)}</span>
                      </div>
                      {damageInsuranceAmount > 0 && (
